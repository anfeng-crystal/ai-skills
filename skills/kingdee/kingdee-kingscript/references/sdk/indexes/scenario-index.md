...
| 页面提示与确认回调 | `IFormView`、`MessageBoxClosedEvent`、`showMessagesAndConfirmCallback` | 先区分普通提示、确认框回调和子页面关闭回调，不混用事件参数 |
...
| 事件参数类型校验 | `error-index`、`plugin-index`、`AbstractFormPlugin`、`AbstractBillPlugIn`、`AbstractListPlugin` | 先确认插件基类和生命周期，再确定事件参数对象；声明层已有明确类型时不要退回 `any` |
