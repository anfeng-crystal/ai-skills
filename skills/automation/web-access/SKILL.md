...
- 按副作用而非 click、CDP 或 dry-run 名称判断。完成已授权任务所必需的只读导航、展开、翻页、无业务写入交互和会话读取可连续完成，无需用户逐个点名；先核实并复用有效登录态，明确要求的下载按目标与范围执行。
...
- 限流先在原范围内退避、缩量或换已授权来源；不绕过验证码或访问控制。需要用户本人交互的阻塞只暂停对应步骤或来源，继续核验其他独立且已授权的来源。
