...
> 先执行 `SKILL.md` 的目标版本检查；本页新旧 API 分支是查证线索，不自动决定项目版本。接口已由当前目标证据确认时复用该依据，缺失时只暂停相关目标调用的实现。
...
下列保留已有参考中的版本分支；使用前仍需目标项目/平台依赖、声明或同目标版本官方签名确认，不能仅因 `ClickHouseClient` 出现在新版参考或 manifest 就给 7.0 工程换用。此处未补充新的产品适用版本证明。
