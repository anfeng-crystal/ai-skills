...
- DataLoader 的 `candidateKeys` 必须是目标实体中已由元数据确认、且本次每个待保存对象均已赋值的业务字段；不得依赖空候选键或编造 `key`、源 UUID、单据号或未证实字段。
- 用户要求 `number`、`billno` 由平台生成，或实体启用 `CodeNumber` 时，不将它们写入映射、候选键或跳过编号校验参数；新增对象不带这两个字段，已有对象先用已验证业务唯一字段定位，再由标准 `save` 生成编号。
- 要求幂等时，先按候选键窄查，确认来源唯一且目标无多命中；仅目标缺失或业务字段实际变化时进入 `save`。候选键无法确认时输出 `contract_incomplete`，不能用主键 `id` 或单据号替代。
- 出现“候选键不存在/未赋值”时，回读当前 DTS 的 `candidateKeys`、映射对象与实体候选字段再修改；不要补写平台生成的 `number`/`billno` 掩盖错误。
- 列表显示数值格式异常时，先区分数据库数值与页面格式，窄查目标 `numeric` 的文本表示和精度，再用元数据定位列表格式；展示问题不执行无效生产 DML，也不把 numeric 改为文本。

## `bizQuery` 连接对象

- `bizQuery` 首参必须是当前上下文实际提供的 `ConnectionWrapper`（如 `$src`、`$tar`、`$this`），不能传 `'ierp'`、`'cn'` 等字符串；集合、`requires` 和作用域按当前目标版本证据确认。
