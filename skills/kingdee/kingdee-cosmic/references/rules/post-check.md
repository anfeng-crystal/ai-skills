...
- **Gradle 项目** → 先执行 `./gradlew :module:compileJava` 真实编译，成功后继续执行 `cosmic-post-lint.py` 场景/风格校验
...
python3 -m pip install -r "$KINGDEE_COSMIC_SKILL_ROOT/requirements.txt"
```
...
生成代码
  └→ 执行 cosmic-post-check.py
        ...
        │           ├→ 编译成功 → post-lint → 汇总退出状态
        │           └→ 编译失败 → 有区分性证据且风险未上升时修复并复检
        │
        └→ 否 → 回退 cosmic-post-lint.py
                ├→ 有 ERROR?      是 → 有区分性证据且仍在契约内时修复并复检
