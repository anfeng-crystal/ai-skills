...
## 案例二十二：表单插件 NullPointerException - afterBindData 生命周期

### 场景描述
表单插件在 `afterBindData` 方法中访问字段值时抛出 `NullPointerException`，导致表单加载失败。

### 典型错误特征
- **错误信息**：`java.lang.NullPointerException at com.example.FormPlugin.afterBindData(FormPlugin.java:45)`
- **触发时机**：表单打开、刷新或切换单据时
- **涉及生命周期**：`afterBindData` - 数据绑定完成后，但控件可能尚未完全初始化

### 根本原因
在 `afterBindData` 阶段，虽然数据已绑定到表单，但以下情况可能导致 NPE：
1. **动态字段未加载**：动态表单字段在此阶段可能尚未创建
2. **分录体为空**：新建单据时分录体集合为空，直接访问会 NPE
3. **辅助资料未展开**：基础资料字段的辅助属性（如 `name`、`number`）可能未加载
...
public void afterBindData(EventObject e) {
    ...
    String customerName = customer.getString("name"); // NPE!
    
    // 错误：未判断分录体是否为空
    DynamicObjectCollection entries = this.getModel().getEntryEntity("entry");
    DynamicObject firstEntry = entries.get(0); // NPE!
}
```
...
public void afterBindData(EventObject e) {
    ...
    if (entries != null && !entries.isEmpty()) {
        DynamicObject firstEntry = entries.get(0);
        // 处理逻辑
    }
    
    // 正确：使用安全的字段访问方式
    String materialNumber = this.getModel().getValue("material.number") != null 
        ? this.getModel().getValue("material.number").toString() 
        : "";
}
```

### 预防措施
1. **afterBindData 阶段必须判空**：所有字段访问前先判断 `!= null`
2. **分录体访问前检查**：`entries != null && !entries.isEmpty()`
3. **基础资料字段使用安全访问**：通过 `getValue("field.subfield")` 并判空
4. **动态字段延迟访问**：如需访问动态字段，在 `afterCreateNewData` 或用户交互事件中处理

---

## 案例二十三：表单插件 NullPointerException - beforeDoOperation 生命周期

### 场景描述
表单插件在 `beforeDoOperation` 方法中校验数据时抛出 `NullPointerException`，导致保存、提交等操作失败。

### 典型错误特征
- **错误信息**：`java.lang.NullPointerException at com.example.FormPlugin.beforeDoOperation(FormPlugin.java:78)`
- **触发时机**：点击保存、提交、审核等操作按钮时
- **涉及生命周期**：`beforeDoOperation` - 操作执行前的校验阶段

### 根本原因
在 `beforeDoOperation` 阶段进行业务校验时，常见 NPE 原因：
1. **必填字段未填写**：用户未填写必填字段，直接访问导致 NPE
2. **分录体未录入**：单据无分录明细，访问分录字段 NPE
3. **关联单据未加载**：上游单据字段未完整加载
...
public void beforeDoOperation(BeforeDoOperationEventArgs args) {
    if ("save".equals(args.getOperationKey())) {
        ...
        String warehouseNumber = warehouse.getString("number"); // NPE!
        
        // 错误：未判断分录体
        DynamicObjectCollection entries = this.getModel().getEntryEntity("entry");
        BigDecimal totalQty = entries.stream()
            .map(e -> e.getBigDecimal("qty"))
            .reduce(BigDecimal.ZERO, BigDecimal::add); // NPE!
    }
}
```
...
public void beforeDoOperation(BeforeDoOperationEventArgs args) {
    if ("save".equals(args.getOperationKey())) {
        ...
        if (entries == null || entries.isEmpty()) {
            args.setCancel(true);
            args.setCancelMessage("至少需要一条分录明细");
            return;
        }
        
        // 正确：安全计算分录汇总
        BigDecimal totalQty = entries.stream()
            .filter(e -> e.getBigDecimal("qty") != null)
            .map(e -> e.getBigDecimal("qty"))
...
### 预防措施
1. **beforeDoOperation 必须做完整性校验**：所有业务规则校验前先判空
2. **使用 `args.setCancel(true)` 阻止操作**：发现数据不完整时明确提示用户
3. **分录体校验模式**：`entries == null || entries.isEmpty()` 双重判断
4. **关联字段延迟加载**：如需访问上游单据，使用 `BusinessDataServiceHelper` 重新加载
...
### 场景描述
操作插件中调用多个保存服务，其中一个失败导致事务回滚，但部分数据已写入缓存或发送到外部系统，造成数据不一致。

### 典型错误特征
- **错误信息**：`Transaction rolled back` 或 `Deadlock detected`
- **触发时机**：批量操作、跨单据保存、调用外部接口时
- **表现**：数据库无数据但缓存有数据，或外部系统已收到通知但苍穹单据未保存成功

### 根本原因
1. **事务边界不清晰**：在同一事务中混合数据库操作和外部调用
2. **缓存未感知事务**：本地缓存写入不受事务控制
3. **外部调用不可回滚**：HTTP 请求、MQ 消息发送无法随事务回滚
...
public void processOrder(DynamicObject order) {
    ...
    // 外部调用无法回滚
    httpClient.post("https://external-api.com/notify", order); // 已发送！
    
    // 如果后续操作失败，事务回滚，但外部系统已收到通知
    updateInventory(order); // 抛异常，事务回滚
}
```
...
### 场景描述
多个插件挂载到同一表单的同一事件点，导致部分插件逻辑不执行或相互覆盖。

### 典型错误特征
- **表现**：某个插件的逻辑不生效，但代码和配置都正确
- **触发时机**：多个二开项目合并、插件热部署后
- **涉及场景**：表单插件、操作插件、服务插件

### 根本原因
1. **插件优先级冲突**：多个插件挂载到同一事件点，执行顺序不确定
2. **事件拦截**：某个插件调用 `args.setCancel(true)` 阻止了后续插件执行
3. **返回值覆盖**：多个插件修改同一字段，后执行的插件覆盖前面的结果
...
```sql
-- 1. 查询表单的所有插件挂载
SELECT * FROM t_meta_formplugin 
WHERE FFORMID = 'bd_material' 
ORDER BY FSEQ;

-- 2. 查询操作插件挂载
SELECT * FROM t_meta_operationplugin 
WHERE FOPERATIONID = 'save' 
ORDER BY FSEQ;

-- 3. 检查插件是否启用
SELECT * FROM t_sec_plugin 
WHERE FPLUGINID IN ('plugin1', 'plugin2');
```

**解决方案**：
1. **调整插件优先级**：在元数据中设置 `FSEQ` 字段，控制执行顺序
2. **合并插件逻辑**：将冲突的插件合并为一个，统一处理
3. **使用插件链模式**：在插件中调用 `super.method()` 确保父类逻辑执行
4. **避免滥用 `setCancel`**：只在必要时阻止操作，不影响其他插件
...
public void beforeDoOperation(BeforeDoOperationEventArgs args) {
    ...
    if (needValidation() && !isValid()) {
        args.setCancel(true);
        args.setCancelMessage("校验失败");
        return;
    }
    
    // 正确：不覆盖其他插件的结果，追加处理
    String existingMessage = args.getCancelMessage();
    if (existingMessage != null) {
        args.setCancelMessage(existingMessage + "; 额外校验失败");
    }
}
```

### 预防措施
1. **插件注册时检查冲突**：开发阶段查询已有插件，避免重复挂载
2. **使用插件优先级**：明确设置 `FSEQ`，确保执行顺序
3. **插件职责单一**：一个插件只处理一类业务逻辑，避免功能重叠
4. **测试多插件场景**：集成测试时模拟多插件共存情况
