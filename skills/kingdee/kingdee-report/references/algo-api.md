...
报表取数 Algo/DataSet 常用 API 的签名检索候选，按功能分组；本文件未逐项注明适用版本。生成或修改目标代码前，按 [SKILL.md](../SKILL.md) 已确认的产品/版本，用目标项目依赖、匹配目标实际版本的 SDK/Javadoc 或声明核对所用 API、完整重载与返回类型。新版或同大版本另一补丁的签名不能自动用于当前目标；已有有效目标证据可复用，不能等冲突出现后才核对。
