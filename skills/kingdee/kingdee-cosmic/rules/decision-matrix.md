...
卡片、模板与示例用于定位候选；生成平台代码前按 [SKILL.md 的契约](../SKILL.md#契约) 复用已确认目标版本并核对本次 API，不以资料版本替换项目目标。
...
- **生成前核对**: `--full` 返回清晰 Java 示例只能提供接口、`@Override`、参数和返回值候选；先用目标项目依赖或匹配目标版本的 SDK/Javadoc 确认本次签名，再生成实现。已有有效证据足够时不强制调用 `cosmic-api-knowledge.py detail`；仍不足且该查询适用时，可用 `detail <full.class.Name>` 补证，但其返回也须核对产品与版本
- **约束**: 仅用于定位标准产品预留的业务拓展点候选；不要误判为 OpenAPI 控制器、表单插件或操作插件。清晰示例、新版资料或同一大版本的另一补丁均不自动证明目标 API 可用；未知签名只暂停依赖实现，继续独立本地工作
...
3. **用户想找标准产品预留扩展点**：优先命中 [`#business-extpoints`](#business-extpoints)，用业务关键词查候选接口，按该卡片核对目标签名；已有有效依赖证据时不固定追加 `detail`。
4. **仍无法匹配**：结合相关类检索和项目同类实现判断；需要在线 `cosmic-api-knowledge.py search <关键词>` 时遵守当前环境与授权边界。证据足以支持当前已授权目标的最小实现时继续；只有仍缺少必要业务信息，或存在会改变业务行为、交付范围或已批准方案的多个方向时，才向用户确认。公共接口、依赖、文件结构及其他既有明确批准要求仍照常执行。
