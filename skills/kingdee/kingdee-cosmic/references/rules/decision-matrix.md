...
- 卡片、模板与示例只定位候选；生成平台代码前按 [SKILL.md 的契约](../../SKILL.md#契约) 复用已确认目标版本并核对本次 API，不以资料版本替换项目目标。业务拓展点签名与当前兜底规则见 [主决策矩阵](../../rules/decision-matrix.md)。
...
3. **仍无法匹配**：按 [主决策矩阵的未命中兜底](../../rules/decision-matrix.md#fallback) 复用项目证据并继续授权范围内的独立工作；只在必要业务信息或会改变行为、范围、已批准方案的方向仍缺失时询问，不因表格未命中或签名待查而逐步骤重复确认。
