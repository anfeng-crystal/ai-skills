...
def _resolve_cookie_file(explicit_cookie_file: Optional[str], urls: List[str]) -> Optional[str]:
    configured = (explicit_cookie_file or os.getenv("YTDLP_COOKIES_FILE") or "").strip()
    if not configured:
        return None

    if not explicit_cookie_file and not any(_is_youtube_url(url) for url in urls):
        return None

    resolved = Path(configured).expanduser()
    if not resolved.exists():
        raise FileNotFoundError(f"Configured cookies file not found: {resolved}")
    return str(resolved)
