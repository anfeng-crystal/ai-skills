...
- 设计期扩展/继承文件记录差量，运行期效果由父元数据与差量叠加；单份差量缺字段不是不存在证据。否定字段存在前，核对目标对象的完整有效链或运行期元数据。扩展通常沿用原实体入口和标识，继承实体有独立标识/入口，不能仅凭共表把两者合并。依据：[扩展继承开发指南](https://vip.kingdee.com/knowledge/specialDetail/218022218066869248?category=475350052731486720&id=312528078846930944&type=Knowledge&productLineId=29&lang=zh-CN)，官方知识正文更新于 2026-07-30，未标最低产品版本；具体链以目标环境为准。
