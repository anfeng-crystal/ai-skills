...
python union_search_cli.py download "https://youtu.be/Zh9IscszDQg" --cookies-file /absolute/path/to/cookies.txt --restrict-filenames --continue-download --output-dir ./downloads
```

> 提示：`search` 输出中的 `download_candidates` 字段提供稳定索引，可直接传给 `download --select`。
> 提示：`download` 对 YouTube 只会使用显式 `--cookies-file` 或环境变量 `YTDLP_COOKIES_FILE`，不再自动探测宿主 skill 目录。
