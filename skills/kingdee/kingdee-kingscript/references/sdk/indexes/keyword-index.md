...
| 确认框 | `IFormView` | 页面提示与确认 | `showConfirm` 入口，确认结果再看 `MessageBoxClosedEvent` |
| 确认框回调 | `MessageBoxClosedEvent` | 页面提示与确认 | `confirmCallBack`、`messageBoxClosed` 的事件参数 |
| 消息框关闭事件 | `MessageBoxClosedEvent` | 页面提示与确认 | 不要和 `ClosedCallBackEvent`、`BillClosedCallBackEvent` 混用 |
...
| 事件参数类型不匹配 | `error-index` | 错误与排障 | 先按插件基类和生命周期回到事件参数类 |
| `any` 用错位置 | `error-index` | 错误与排障 | 声明层已有明确类型时不要用 `any` 抹平 |
