...
class ApiGraph:
    ...
    def search_methods(
        ...
        *,
        ...
        conn = self._get_conn()
        if not conn: return f"❌ 未配置 graph.dbPath，或未在路径 '{self.db_path}' 下找到有效数据库"
            ...
            with conn:
                ...
                def append_rows(title: Optional[str], result_rows):
                    ...
                    for row in result_rows:
                        ...
                        simple_cls = row['class_name'].rsplit('.', 1)[-1]
                        cheat_tag = " ✅" if (simple_cls, row['method_name']) in self._cheat_methods else ""
    ...
    def fuzzy_search_classes(
        ...
        *,
        ...
        if not conn:
            ...
            return f"❌ 未配置 graph.dbPath，或未在路径 '{self.db_path}' 下找到有效数据库"
