...
- `author` 只按本地源码、测试、构建和包结构报告；`release` 还必须确认目标版本合同，才能标为可交付的版本化包，版本未确认时仅交本地产物并标 `blocked`/`not-run`。平台运行回滚不可用时，将 `platformInstall`/`runtimeVerification` 标为 `blocked` 或 `not-run`，不得把本地 release 写成平台已验证。`deploy-approved` 才要求目标运行回滚合同可用。
