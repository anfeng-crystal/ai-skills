...
2. 对照当前 DTS，列出 missing、extra、类型不符、语义来源不符和空值策略不符；用户要求修复时，同步修改 DTS，不得只修测试 JSON。
...
- 用户提供历史成功 payload 时，先把它与最新字段说明、当前错误响应和当前元数据合并成一份带版本/哈希的合同快照；当前用户纠正优先，历史成功 payload 只作为其未覆盖部分的类型与形态证据。
- 在修改 DTS 前，对所有目标主出站流程一次性完成 key、JSON 类型、语义表示、编码体系、空值策略和来源的全量 diff。只有全量 diff 证明其余字段一致时，才可把修复收敛到单个字段。
- 任一多选基础资料、复选框、枚举、数值或日期出现类型错误时，必须扫描两个目标主出站流程中所有同元数据类型字段；不能只修报错栈中首先出现的字段。
- 多选源映射到单选目标时，合同必须明确选取顺序、空值和多余项去向；未明确时停止，不擅自取第一项、拼接或改数组。
- 路由/落表判别字段与业务状态字段分别取证。即使当前编码碰巧相同，也不能复用同一变量；只有合同明确同源时才能复用。
- 每次用户纠正字段形态或业务语义后，旧合同快照和基于它生成的候选 DTS 立即失效，按最新口径重新全量对账。
