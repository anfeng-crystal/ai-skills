...
6. apply 后立即验证：`cosmic_libs_path` 指向目标目录，项目检测到的 Gradle wrapper 与 debug/test task 通过（POSIX `./gradlew`，Windows `gradlew.bat`），本地首页至少能到登录页；如失败，执行 `rollback` 或按报告恢复 `.quarantine`。
